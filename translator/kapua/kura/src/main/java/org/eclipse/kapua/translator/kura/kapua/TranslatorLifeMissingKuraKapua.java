...
public class TranslatorLifeMissingKuraKapua extends Translator<KuraMissingMessage, KapuaMissingMessage> {
    ...
    public KapuaMissingMessage translate(KuraMissingMessage kuraMissingMessage)
        ...
        Account account = KapuaSecurityUtils.doPriviledge(() -> accountService.findByName(kuraMissingMessage.getChannel().getScope()));
        ...
        Device device = KapuaSecurityUtils.doPriviledge(() -> deviceRegistryService.findByClientId(account.getId(), kuraMissingMessage.getChannel().getClientId()));
