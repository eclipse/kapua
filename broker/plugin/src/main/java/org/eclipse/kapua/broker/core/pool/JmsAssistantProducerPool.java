...
public class JmsAssistantProducerPool extends GenericObjectPool<JmsAssistantProducerWrapper> {
    ...
    protected JmsAssistantProducerPool(JmsAssistantProducerWrapperFactory factory) {
        super(factory);
        int totalMaxSize = BrokerPluginSetting.getInstance().getInt(BrokerPluginSettingKey.BROKER_CLIENT_POOL_NO_DEST_TOTAL_MAX_SIZE, 10);
        int maxSize = BrokerPluginSetting.getInstance().getInt(BrokerPluginSettingKey.BROKER_CLIENT_POOL_NO_DEST_MAX_SIZE, 10);
        int minSize = BrokerPluginSetting.getInstance().getInt(BrokerPluginSettingKey.BROKER_CLIENT_POOL_NO_DEST_MIN_SIZE, 5);
