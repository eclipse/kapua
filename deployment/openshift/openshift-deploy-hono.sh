...
${OC} new-app -n "${OPENSHIFT_PROJECT_NAME}" -f templates/kapua-template-hono-processors.yml     -p "DOCKER_ACCOUNT=${DOCKER_ACCOUNT}" -p "IMAGE_VERSION=${IMAGE_VERSION}" -p "JAVA_OPTS_EXTRA=${JAVA_OPTS_EXTRA}" -p "HONO_ADDR=${HONO_ADDR}"
