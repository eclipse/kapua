...
public class DeviceTabInventoryTabInventory extends TabItem {
    ...
    private Grid<GwtInventoryItem> grid;
    private ListLoader<ListLoadResult<GwtInventoryItem>> storeLoader;

    public DeviceTabInventoryTabInventory(DeviceTabInventory parentTabPanel) {
        super("Inventory", new KapuaIcon(IconSet.DROPBOX));
    ...
    protected void onRender(Element parent, int index) {
        ...
        RpcProxy<ListLoadResult<GwtInventoryItem>> proxy = new RpcProxy<ListLoadResult<GwtInventoryItem>>() {

            @Override
            protected void load(Object loadConfig, AsyncCallback<ListLoadResult<GwtInventoryItem>> callback) {
                GWT_DEVICE_INVENTORY_MANAGEMENT_SERVICE.findDeviceInventory(getSelectedDevice().getScopeId(),
                        getSelectedDevice().getId(),
                        callback);
            }
        };

        storeLoader = new BaseListLoader<ListLoadResult<GwtInventoryItem>>(proxy);
        storeLoader.addLoadListener(new InventoryLoadListener());

        ListStore<GwtInventoryItem> store = new ListStore<GwtInventoryItem>(storeLoader);

        grid = new Grid<GwtInventoryItem>(store, columnModel);
    ...
    private class InventoryLoadListener extends KapuaLoadListener {
