...
import org.eclipse.kapua.app.console.shared.service.GwtJobTargetServiceAsync;
...
public class JobTargetAddDialog extends EntityAddEditDialog {
    ...
    public void submit() {
        if (targetRadioGroup.getValue().getValueAttribute().equals(RadioGroupStatus.SELECTED.name())) {
            ...
            GWT_DEVICE_SERVICE.query(new GwtDeviceQuery(currentSession.getSelectedAccount().getId()), new AsyncCallback<List<GwtDevice>>() {
    ...
    private void doSubmit(List<GwtDevice> targets) {
        ...
        for (GwtDevice target : targets) {
            ...
            creator.setScopeId(currentSession.getSelectedAccount().getId());
