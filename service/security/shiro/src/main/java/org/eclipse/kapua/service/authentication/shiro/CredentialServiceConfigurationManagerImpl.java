...
||||||| parent of bc801905b0 (:fix: integration tests)
=======
/*******************************************************************************
 * Copyright (c) 2016, 2022 Eurotech and/or its affiliates and others
 *
...
public class CredentialServiceConfigurationManagerImpl extends ServiceConfigurationManagerBase implements CredentialServiceConfigurationManager {
    ...
    @Override
    protected boolean validateNewConfigValuesCoherence(KapuaTocd ocd, Map<String, Object> updatedProps, KapuaId scopeId, Optional<KapuaId> parentId) throws KapuaException {
        if (updatedProps.get(CredentialServiceImpl.PASSWORD_MIN_LENGTH) != null) {
            // If we're going to set a new limit, check that it's not less than system limit
            int newPasswordLimit = Integer.parseInt(updatedProps.get(CredentialServiceImpl.PASSWORD_MIN_LENGTH).toString());
            if (newPasswordLimit < systemMinimumPasswordLength || newPasswordLimit > CredentialServiceImpl.SYSTEM_MAXIMUM_PASSWORD_LENGTH) {
                throw new KapuaIllegalArgumentException(CredentialServiceImpl.PASSWORD_MIN_LENGTH, String.valueOf(newPasswordLimit));
            }
        }
        return true;
    }
...
>>>>>>> bc801905b0 (:fix: integration tests)
