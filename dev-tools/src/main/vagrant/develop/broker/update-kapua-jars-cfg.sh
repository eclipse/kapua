...
for name in $(find /kapua -name 'kapua-*.jar' | grep target | grep -Ev 'bin|test|console|WEB-INF|dependency|mysql|assembly|dev-tool|job|scheduler'); do jar_name=$(echo - $name - $name | awk -F"/" '{print $NF}'); echo create symbolic link from ./lib/extra/${jar_name} ${name}; ln -s $name ./lib/extra/$jar_name; done;
