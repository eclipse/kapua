...
for name in $(ls /kapua/broker-core/target/dependency/ | grep -Ev 'jaxb-|activemq-|org.eclipse.kapua'); do echo create symbolic link from ./lib/extra$name /kapua/broker-core/target/dependency/$name; ln -s /kapua/broker-core/target/dependency/$name ./lib/extra/$name; done;
echo '===> copy dependencies for locator/guice'
for name in $(ls /kapua/locator/guice/target/dependency/ | grep -Ev 'jaxb-|activemq-|org.eclipse.kapua'); do echo create symbolic link from ./lib/extra/$name /kapua/locator/guice/target/dependency/$name; ln -s /kapua/locator/guice/target/dependency/$name ./lib/extra/$name; done;
...
echo '===> copy kapua modules'
for name in $(find /kapua -name 'kapua-*.jar' | grep target | grep -Ev 'test|console|api|dependency|mysql'); do jar_name=$(echo - $name - $name | awk -F"/" '{print $NF}'); echo create symbolic link from ./lib/extra/${jar_name} ${name}; ln -s $name ./lib/extra/$jar_name; done;
