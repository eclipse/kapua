...
public class DefaultGenericReader extends AbstractItemReader implements TargetReader {

    private static final Logger LOG = LoggerFactory.getLogger(DefaultGenericReader.class);
    ...
    public Object readItem() throws Exception {
        KapuaJobContext kapuaJobContext = JOB_CONTEXT_FACTORY.newJobContext(jobContext);
        LOG.info("JOB {} - Reading items...", kapuaJobContext.getJobId());

        if (executed) {
            return null;
        }

        LOG.info("JOB {} - Reading items... Done!", kapuaJobContext.getJobId());
        executed = true;
        return jobTargets;
    }
}
