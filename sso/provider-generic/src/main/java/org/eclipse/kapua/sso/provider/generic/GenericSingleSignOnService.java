...
public class GenericSingleSignOnService extends AbstractSingleSignOnService  {
    ...
    private final static Logger logger = LoggerFactory.getLogger(GenericSingleSignOnService.class);
    ...
    public String getLoginUri(final String state) {
        try {
            final URIBuilder uri = new URIBuilder(genericSettings.getString(GenericSsoSettingKeys.SSO_OPENID_SERVER_ENDPOINT_AUTH));

            uri.addParameter("scope", "openid");
            uri.addParameter("response_type", "code");
            uri.addParameter("client_id", genericSettings.getString(GenericSsoSettingKeys.SSO_OPENID_CLIENT_ID));
    ...
    public JsonObject getAccessToken(final String authCode) throws IOException {
        ...
        final URL url = new URL(genericSettings.getString(GenericSsoSettingKeys.SSO_OPENID_SERVER_ENDPOINT_TOKEN));
        ...
        parameters.add(new BasicNameValuePair("client_id", genericSettings.getString(GenericSsoSettingKeys.SSO_OPENID_CLIENT_ID)));

        final String clientSecret = genericSettings.getString(GenericSsoSettingKeys.SSO_OPENID_CLIENT_SECRET);
